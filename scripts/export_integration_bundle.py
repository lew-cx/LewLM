...
CHAP_FIELD_NOTES = {
    ...
    "service_vs_engine_vs_model": "`GET /v1/health.status` is this service. Engine reachability is `health.engines[].state` and `GET /v1/runtime.startup.engines[]` (cached inventory: advertised | stale | failed | unknown; no probe). A request LewLM finds refused marks its endpoint `stale` (or `failed` if never read) at once, with `inventory_error`; the next successful read restores `advertised`. Model warmth is `runtime.startup.warm_models[]` / `GET /v1/runtime/residencies`. A 200 from health never implies an engine is up or a model is warm.",
    "model_picker": "`GET /v1/models.capability_availability[]` gives, per model: `chat_ready`, `reason`, `endpoint_id`/`engine_profile`/`execution_locality` (null for packaged runtimes), and `engine_state` (`packaged`, or the endpoint's cached state). When the engine is down, `reason` names the endpoint, its state and error; `fallback_model_id` is set when an explicit fallback alias would answer a chat for the model anyway (`chat_ready` stays false). `GET /v1/models/{id}/capabilities.structured_output` predicts enforcement before a request is spent; `capabilities.tool_calling.support` (`native` | `prompt_guided` | `none`) says whether and how declared tools reach the model, and `parallel` whether one reply can carry several calls (`null`: the engine decides).",
    ...
    "streaming": "SSE `data:` frames of `chat.stream` chunks, then `data: [DONE]`. Exactly one chunk carries a non-null `finish_reason` (`stop`, `length`, `tool_calls`, `cancelled`, or `error`). Native tool-call fragments arrive as `delta.tool_calls[]` with `index` for live rendering; the terminal chunk's `tool_calls` carries LewLM's parsed, schema-validated verdict, exactly as the sync body does, so a client never reassembles fragments.",
    "incomplete_stream": "If generation fails after output has started, the stream still returns HTTP 200: the last chunk has `finish_reason: \"error\"` and an `error` envelope (`code`, `message`, redacted `details`, `partial_output: true`), followed by `[DONE]`. Delivered text stands; LewLM never replays. A cancelled stream ends with `finish_reason: \"cancelled\"`. A stream refused before any output (an unreachable engine) is not opened: it gets the same HTTP error as the sync request.",
    ...
    "events": "`GET /v1/events` (SSE) and the WebSocket carry the same filters (`types`, `scope`, `request_id`, `model_id`, `exclude_types`). Every frame has a cursor (`id:` line; `cursor` in the body). Resume with `Last-Event-ID` or `?after=`: the stream starts with `events.resumed` (`replayed`, `lost` — `null` for a cursor from another server lifetime — `latest`), then the replayed events under their original cursors, then live ones. Retained: `LEWLM_EVENT_REPLAY_BUFFER_SIZE` (4096) events across every type.",
    "tool_results": "Continue a tool call by sending the assistant turn back with `tool_calls` (`[{id, type: \"function\", function: {name, arguments}}]`, `id` = the reported `call_id`; `content` may be null) and one `tool` message per result with `tool_call_id`. Engines that take these fields get them; packaged runtimes see the calls rendered in the prompt's tool-call shape. `tool_call_id` is refused on any role but `tool`, and `tool_calls` on any but `assistant`.",
