...
Audio capability is per model, not per runtime: a manifest carries `audio_roles`, and the inventory's `ready_capabilities` names only the side a model serves, so a synthesis request against a transcription model is a `routing_error` rather than a `500`. `GET /v1/audio/voices` reports the voices a synthesis model can resolve **on this host**, since a backend may keep its voice packs in its own download cache rather than in the model directory. A listed voice is a guarantee; an absent one is not a refusal, because a backend may fetch a name on demand. The same response lists `formats[]` — the encodings `POST /v1/audio/speech` can return for that model on this host, each with its `media_type` and whether it is `verified` (LewLM encodes it itself or has observed the runtime return it) or merely forwarded to the backend — plus `default_format` (`wav`) and `formats_exhaustive`. When the list is exhaustive (native MLX audio: `wav` always, `mp3`/`flac`/`ogg` only through mlx-audio's own helper), a request naming any other `format` is refused as `422 invalid_request` with `details.accepted_formats` before any model is loaded; a bridge forwards the name as `response_format` and its upstream decides, so its list is open and `wav` becomes verified once the speech probe has returned audio.
...
Both surfaces accept the same filters as query parameters: `types`, `scope`, `request_id`, `model_id` and `exclude_types`. Each may be repeated or comma-separated, values within one parameter are alternatives, and the parameters combine — `?types=token.delta&request_id=req-1` is one request's tokens and nothing else. Filtering is applied before an event is queued for the connection, so an excluded event is never serialized or sent. A value that names no known event type or scope is refused with `invalid_request` (422 on SSE, close code `1008` before the WebSocket handshake is accepted) rather than silently ignored, because an ignored filter returns an empty stream that looks exactly like a quiet server.

`exclude_types` is the one negative filter, applied after `types`: `?exclude_types=token.delta,reasoning.delta` is everything but the per-token flood, without spelling out the other fifty types.

Every frame carries a cursor: the SSE `id:` line, and `cursor` in the event body on both surfaces. It is opaque (`<epoch>:<sequence>`; compare only for equality) and is the value a reconnecting client sends back — as `Last-Event-ID`, which an `EventSource` does on its own reconnect, or as `?after=` from a client that cannot set headers; the header wins when both are present. A resumed stream begins with one `events.resumed` frame (no `id:`, so it never advances the client's cursor) whose payload says what the replay could do: `replayed` (retained events newer than the cursor that the filter admitted, delivered next, in order, under their original cursors, before anything live), `lost` (events of any type published after the cursor but no longer retained; `null` when the cursor is from another server lifetime, in which case nothing is replayed because nothing since it is knowable), `latest`, `retained` and `replay_buffer_size`. An event published while the client reconnects is either replayed or delivered live, never both and never neither. A cursor this server did not issue, or one ahead of the stream, is refused as `invalid_request` before the stream opens. The buffer holds the last `LEWLM_EVENT_REPLAY_BUFFER_SIZE` events (default 4096, across every type, so a streaming generation's token deltas consume it fastest); a client that was away longer than that sees an honest `lost` count rather than a silent gap.
